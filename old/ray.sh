...
# 原限制ubuntu18以上，解除限制到14以上，能不能用没测过
#====================================================
...
function acme() {
# patch 增加dns方式申请证书，Cloudflare，需设置CF_Token变量
  "$HOME"/.acme.sh/acme.sh --set-default-ca --server letsencrypt

  sed -i "6s/^/#/" "$nginx_conf"
  sed -i "6a\\\troot $website_dir;" "$nginx_conf"
  systemctl restart nginx
  if [[ -n $CF_Token ]] && "$HOME"/.acme.sh/acme.sh --issue -d "${domain}" --dns dns_cf -k ec-256 --force;then
    print_ok "SSL 证书生成成功"
    sleep 2
  ...
  elif "$HOME"/.acme.sh/acme.sh --issue --insecure -d "${domain}" --webroot "$website_dir" -k ec-256 --force; then
    print_ok "SSL 证书生成成功"
    sleep 2
    if "$HOME"/.acme.sh/acme.sh --installcert -d "${domain}" --fullchainpath /ssl/xray.crt --keypath /ssl/xray.key --reloadcmd "systemctl restart xray" --ecc --force; then
      print_ok "SSL 证书配置成功"
...
function warp_install() {
  wget -N --no-check-certificate "https://github.com/xiaofd/xiaofd.github.io/raw/master/others/warp.sh" && chmod +x warp.sh && bash warp.sh wg6
}
function patch_google(){
...
menu() {
  ...
  echo -e "${Green}15.${Font} 安装warp PATCH google"
  echo -e "—————————————— 查看信息 ——————————————"
