...
[Service]
Type=oneshot
ExecStart=/bin/bash -c 'ip route add default via ${NetoutGateway} dev ${NetDevice} table table_1; ip route add default via 192.168.0.1 dev ${NetDevice} table table_2; ip rule add from 192.168.1.0/24 iif $(ip route | grep 192.168.1.0 | awk "{print \$3}") lookup table_1; ip rule add from 192.168.2.0/24 iif $(ip route | grep 192.168.2.0 | awk "{print \$3}") lookup table_2;sudo ip -6 rule add from 2001:db8:1::/64 iif $(ip -6 route | grep 2001:db8:1:: | awk '{print $3}') lookup table_1;sudo ip -6 rule add from 2001:db8:2::/64 iif $(ip -6 route | grep 2001:db8:2:: | awk '{print $3}') lookup table_2'
...
systemctl start apply-routes.service
